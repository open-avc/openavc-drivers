...
- `id_format.type` is `integer` (default) or `string`. For `integer`: `min` defaults to 1, `max` is optional (unbounded if omitted), `pad_width` zero-pads the ID in state keys (`encoder.005`) and must not be negative — omit it or set it to `0` for no padding. Catalog convention is `pad_width: 2`. For `string`: children are keyed by a device-native name (a Q-SYS Code Name, an MQTT topic leaf) restricted to `[A-Za-z0-9_-]` and at most `max_length` chars (default 128) — sanitize the native name to that charset and keep the original in the child's `label`.
