...
**A command that sends a datagram beside the transport** (a `udp:` block: a Wake-on-LAN magic packet, a message to a presentation's UDP receiver; requires platform 0.34.0) is received too. A TCP or HTTP device that declares one also listens for UDP datagrams on its own port number, so the datagram appears in the Simulator UI's protocol log instead of vanishing. A `payload` runs through the same command pipeline as a line on the transport (its `sets:` apply, its name is inferred the same way); a `magic_packet` applies the command's declared `sets:`, so a wake written as `power_on` with `sets: { power: true }` turns the simulated display on the way the real one comes on. Nothing is answered on the side channel: the driver's socket closed with the send, and a real device replies on its control link.

**Device settings are answered too** (requires platform 0.34.0). A `device_settings` entry's `write` carries the same templates a command's `send` does and its `state_key` says where the value lands, so the simulator builds a handler from the pair: writing a setting from the IDE updates the simulated device and is reported back the way the real one reports it, which is what the platform waits for before the setting stops showing as pending. Two details it handles for you, because real drivers need them: a write that carries `{config_field}` placeholders is substituted the way the driver substitutes it before sending, and a write that is a set followed by a read-back query (two commands joined by the terminator) is matched on the segment carrying the value — the query is answered by whatever answers queries. A setting written over OSC (`address:`) is not covered; OSC is answered from your `responses` address mappings instead. No `simulator:` section is needed for any of this. A driver that already has one keeps winning: an explicit handler matching the same text is used ahead of the generated one.
...
**SSE push drivers.** When an HTTP driver declares `push: {type: sse}`, the simulated device serves the declared event-stream path(s): a GET with `Accept: text/event-stream` on one of them is held open, and every `notifications` template is delivered to all subscribers as a `data: <message>` event when state changes. Author the templates as the JSON bodies the driver's `json: true` rules parse — remember that `{value}` renders a Python boolean as `True`/`False`, so boolean variables need per-value templates carrying the JSON literals (`"true": '{"sharing":true}'`), and a rule scoped with `require:` needs its required key present in the template. Requests without the `Accept: text/event-stream` header route through `command_handlers` normally. See `barco_clickshare_cx` for a full example. Python HTTP simulators (`HTTPSimulator` subclasses) get the same capability by listing `sse_paths` and calling `self.push_sse_event(json_string)`.

With a `session` block on the push shape (the stream is a session the device names, which the driver then subscribes with register commands), the simulated device names each subscription the way the real one does: a fresh id per stream, carried in the declared header as `<path>/<id>` and in the declared opening event's data under the declared key. The driver's register commands then arrive as ordinary requests (`PUT /api/ssc/state/subscriptions/<id>|["/api/device/state"]`), so a `command_handlers` entry matching the path with the id as a capture answers them — and answers like the device by calling `notify(...)` once per subscribed resource with its current value, which is how a real subscription reports everything it was just given. `notifications` templates keyed by child state (`channel.1.mute`) cover later changes. `sennheiser_ewdx` is the full example. Python HTTP simulators set `sse_session` to the same block; `close_sse_sessions()` sends the closing event and drops every subscriber, the way a device does before a reboot.
